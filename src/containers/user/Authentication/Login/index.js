...
export class Login extends Component {
  ...
  async verifyEmail() {
    const {
      match, history, save, t,
    } = this.props;
    try {
      const token = match && match.params && match.params.token;
      if (token) {
        const url = `/api/v1/activate/${token}`;
        const axiosSetup = {
          url,
          method: 'get',
        };
  ...
  async submit(e) {
    e.preventDefault();
    const { t, setId, dispatchSetUserAction } = this.props;
    ...
    if (Validation.isValidated(template1(t).validators)(this.state)
      ...
      try {
        ...
        if (result && result.data) {
          save('userInfo_user', result.data);
          const user = await apiAccount.getMyProfile();
          dispatchSetUserAction(user.data);
          await this.getAPICompany();
          setId();
          if (result.data.isEmptyCompany) {
            history.push('/users/initial-setting');
          } else {
            history.push('/users');
          }
        ...
        toast.error(error.response.data.invalidParams.length !== 0 ? t(error.response.data.invalidParams.map(p => p.message).join('\n')) : t(error.response.data.detail), {
          position: 'top-center',
          autoClose: 2000,
          hideProgressBar: true,
          closeOnClick: true,
...
export default UserWrapper(connect(mapStateToProps, mapDispatchToProps)(translateWrapper('Login')(Login)));
