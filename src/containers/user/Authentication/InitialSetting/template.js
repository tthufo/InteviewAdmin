...
export default t => ({
  validators: [
    ...
    Validation.requiredSelect('country_code', 'country_code'),
    Validation.requiredSelect('financial_year_end_month', 'financial_year_end_month'),
    Validation.number('capital_amount', 'capital_amount'),
    Validation.requiredSelect('language', 'language'),
    Validation.requiredSelect('currency_code', 'currency_code'),
    Validation.number('capital_reserve_ratio', 'capital_reserve_ratio'),
    Validation.negativeNumber('capital_reserve_ratio', 'capital_reserve_ratio'),
    Validation.requiredSelect('display_amount_type', 'display_amount_type'),
    Validation.maxNumber('capital_amount', 'capital_amount', 12),
    Validation.requiredSelect('tax_rate_id', 'tax_rate_id'),
    Validation.requiredSelect('tax_ross_carry_limit', 'tax_ross_carry_limit'),
  ...
  elements: [
    ...
    {
      defaultValue: { label: 'Japan', value: 'JP' },
      type: 'select',
      input_key: 'country_code',
      title: `${t('country_code')} *`,
      friendKeySelects: ['tax_rate_id', 'currency_code'],
      request: async () => {
        const result = await API.companySetting.dropdownList();
        return (result
          && result.data
    ...
    },
    {
      ...
      request: async () => {
        try {
          const result = await API.companySetting.dropdownList();
          const data = result.data.currencies.map(l => ({ label: l.name, value: l.value }));
          data.splice(8, 0, { label: 'EUR', value: 'EUR' });
          return data.map((d, index) => ({ label: d.label, value: d.value, index }));
        } catch (error) {
          return [];
        }
      },
    ...
    },
    {
      ...
      request: async () => {
        try {
          const result = await API.companySetting.taxRates();
